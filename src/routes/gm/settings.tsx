...
function GMSettingsPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      {session && (
        <div className="mb-6 rounded-xl border border-border bg-card p-4">
          <h2 className="mb-3 font-semibold">{t('settings.sessionInfo')}</h2>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">{t('settings.name')}</span><span className="font-medium">{session.room.name}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">{t('settings.roomId')}</span><span className="font-mono text-xs">{session.room.id}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">{t('settings.created')}</span><span>{new Date(session.room.createdAt).toLocaleString()}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">{t('settings.characters')}</span><span>{Object.keys(session.characters).length}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">{t('settings.players')}</span><span>{Object.keys(session.players).length}</span></div>
          </div>
        </div>
      )}
      ...
      <div className="mb-6 rounded-xl border border-border bg-card p-4">
        <h2 className="mb-1 font-semibold">{t('settings.lightSourceDurations')}</h2>
        <p className="mb-4 text-xs text-muted-foreground">{t('settings.lightSourceDescription')}</p>

        <div className="space-y-4">
          <div>
            <label className="mb-1.5 flex items-center justify-between text-sm">
              <span>{'\u{1F525}'} {t('settings.torchDuration')}</span>
          ...
          <div>
            <label className="mb-1.5 flex items-center justify-between text-sm">
              <span>{'\u{1F3EE}'} {t('settings.lanternDuration')}</span>
          ...
          <div>
            <label className="mb-1.5 flex items-center justify-between text-sm">
              <span>{'\u{1F525}'} {t('settings.campfireDuration')}</span>
      ...
      {/* Data Pack Display */}
      <div className="mb-6 rounded-xl border border-border bg-card p-4">
        <h2 className="mb-1 font-semibold">{t('settings.dataPackDisplay')}</h2>
        <p className="mb-4 text-xs text-muted-foreground">{t('settings.dataPackDisplayDescription')}</p>
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 text-sm">
            ...
            />
            <span>{t('settings.showPackMonstersFirst')}</span>
          </label>
          ...
          <label className="flex items-center gap-3 text-sm">
            <input
              ...
              className="h-4 w-4 rounded border-border accent-primary"
            />
            <span>{t('settings.showPackItemsFirst')}</span>
          </label>
        </div>
      ...
      <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-red-400">{t('settings.endSession')}</h2>
            <p className="text-xs text-muted-foreground">{t('settings.endSessionDescription')}</p>
          </div>
          <button
            onClick={() => {
              gmPeer.destroy()
...
function SessionBackupCard({ session }: { session: import('@/schemas/session.ts').SessionState }) {
  ...
  return (
    <div className="mb-6 rounded-xl border border-border bg-card p-4">
      <h2 className="mb-1 font-semibold">{t('settings.sessionBackup')}</h2>
      <p className="mb-4 text-xs text-muted-foreground">{t('settings.sessionBackupDescription')}</p>
      ...
      <div className="flex items-center gap-3 mb-4">
        <button
          ...
          className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90 transition"
        >
          {t('settings.exportCurrentSession')}
        </button>
      ...
      >
        <label className={`flex cursor-pointer items-center justify-center rounded-lg border-2 border-dashed py-4 transition ${
          ...
          <div className="text-center pointer-events-none">
            <p className="text-sm font-medium">{isDragging ? t('common.dropToImport') : t('gm.importSession')}</p>
